 ...
 try
  ...
  begin
   ...
   While length(Str)<>29 do
    Str:=Str+' ';
   Str:=Str+Copy(Planilha.Cells[4,2],1,2)+Copy(Planilha.Cells[4,2],4,2)+'00';
   While length(Str)<>43 do
    Str:=Str+' ';
   Str:=Str+Copy(Planilha.Cells[5,2],1,2)+Copy(Planilha.Cells[5,2],4,2)+'00';
   While length(Str)<>60 do
    Str:=Str+' ';
   Str:=Str+Copy(Planilha.Cells[6,2],1,pos('m',Planilha.Cells[6,2])-1);
   S.Add(Str);
   Lin:=8;
   UltimoAno:='';
   UltimoMes:='';
   ...
   begin
    ...
    try
     ...
     R:=Copy(Planilha.Cells[Lin,1],1,2)+'/';
     If Copy(Planilha.Cells[Lin,1],4,3)='JAN' then
      R:=R+'01/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='FEV' then
      R:=R+'02/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='MAR' then
      R:=R+'03/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='ABR' then
      R:=R+'04/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='MAI' then
      R:=R+'05/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='JUN' then
      R:=R+'06/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='JUL' then
      R:=R+'07/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='AGO' then
      R:=R+'08/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='SET' then
      R:=R+'09/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='OUT' then
      R:=R+'10/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='NOV' then
      R:=R+'11/'
     else If Copy(Planilha.Cells[Lin,1],4,3)='DEZ' then
      R:=R+'12/';
     R:=R+Copy(Planilha.Cells[Lin,1],8,4);
     Data:=StrToDate(R);
    end;
    //Inicializar Dia
    ...
    begin
     If (Planilha.Cells[Lin,2].text<>'NULL') and (Planilha.Cells[Lin,2].text<>'') then
    ...
    //Dado
    If StrToFloatDef(Dado,0)>999 then
     Dado:=' ';
